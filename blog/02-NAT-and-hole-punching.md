...
The allow list isn't global and is specific to the port, otherwise it would be an obvious security flaw. This setup suggests that the router is responsible for accepting the connection, so port forwarding on router level is possible. That would be true if you have unique IP, but carriers add CGNAT.
...
- **Full Cone**: Once an outbound packet is sent to a server (or anywhere, even dead IP), the port it used to sent is opened permanently (until closed). The thing about full cone is that it allows traffic from any IP and port.

- **IP Restricted Cone**: Allows connection from the IP to which the outbound packet was sent. This means that particular IP can communicate with the client via the router on that particular port.
